...
/**
 * 
 * @author prashantj This class would act as a utility class containint all the
 *         statric functions that are neede to be stand alone functions
 * 
 */
...
public class Utilities {
	...
	public static String encrypt(String strToEncrypt) {
		/**
		 * have to implement a encruption algo to encryot the string for tem p use
		 * urlencoded
		 */
	...
	public static String decryptAES(String encrypted) {
		/*
		 * final String seckey = "12345"; final String initVector = "12345"; byte[] key;
		 * MessageDigest sha = null; try { key = seckey.getBytes("UTF-8"); sha =
		 ...
		 * rand.nextBytes(bytes); IvParameterSpec ivSpec = new IvParameterSpec(bytes);
		 * 
		 * 
		 * SecretKeySpec skeySpec = new SecretKeySpec(key, "AES");
		 * 
		 * Cipher cipher = Cipher.getInstance("AES/CBC/PKCS5PADDING");
		 * cipher.init(Cipher.DECRYPT_MODE, skeySpec,ivSpec); byte[] original =
